...
# Asegurar que la carpeta del script esté en el path
# para que los imports de Red_Hotend, Conversor_Hotend, Adaptativo_Worker
# funcionen sin importar desde qué carpeta se corra la GUI
...
# ==============================================================
# NUEVO CÓDIGO LQR ADAPTATIVO (1/4): IMPORTAR EL CEREBRO
# ==============================================================
from Adaptativo_Worker import HiloMatematicoAdaptativo
...
class ControlGUI(QWidget):
    def __init__(self):
        ...
        # ==============================================================
        # NUEVO CÓDIGO LQR ADAPTATIVO (2/4): INICIAR HILO
        # ==============================================================
        self.hilo_adaptativo = HiloMatematicoAdaptativo()
        self.hilo_adaptativo.nuevos_valores_ready.connect(self.enviar_lqr_al_arduino)
        self.hilo_adaptativo.start()
    ...
    # ==============================================================
    # NUEVO CÓDIGO LQR ADAPTATIVO (3/4): ENVIAR LQR A ARDUINO
    # ==============================================================
    def enviar_lqr_al_arduino(self, K_lista, Nbar):
        """
        Recibe las matrices desde el Hilo Matemático y las envía por el puerto Serial.
        """
        try:
            k_str = ",".join(f"{k:.4f}" for k in K_lista)
            trama = f"LQRH:{k_str},{Nbar:.4f}\n"
            arduino.write(trama.encode('utf-8'))
            print(f"💻 -> 🤖 Adaptación LQR enviada al Arduino: {trama.strip()}")
        except Exception as e:
            print(f"⚠️ Error intentando mandar LQR: {e}")
    ...
    def actualizar(self):
        ...
        # ==============================================================
        # NUEVO CÓDIGO LQR ADAPTATIVO (4/4): ALIMENTAR A LA RED
        # ==============================================================
        # Extraemos las temperaturas y los estados del calentador para la ventana W=100
        W = 100
        min_len = min(len(self.temp_data), len(self.heater_state_data))
        if min_len >= W:
            ultimos_datos = []
            for i in range(min_len - W, min_len):
                t = self.temp_data[i]
                # Convertimos el estado (0 o 1) a PWM aproximado (0 o 255)
                pwm = self.heater_state_data[i] * 255.0
                ultimos_datos.append([t, pwm])

            # Inyectamos de forma segura sin trabar la GUI
            self.hilo_adaptativo.actualizar_ventana(ultimos_datos)
    ...
    def closeEvent(self, event):
        ...
        # Apagar el hilo adaptativo limpiamente
        if hasattr(self, 'hilo_adaptativo'):
            self.hilo_adaptativo.stop()
