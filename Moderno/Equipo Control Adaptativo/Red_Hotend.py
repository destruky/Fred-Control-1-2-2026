...
TRAIN_FILES = [
    BASE / "3)hotend60.csv",
    BASE / "1)hotend100.csv",
    BASE / "2)hotend180.csv",
]
TEST_FILE = BASE / "4)hotend220.csv"
...
def train_phase1(model, train_dl, val_dl, epochs=P1_EPOCHS, lr=P1_LR):
    ...
    for epoch in range(epochs):
        ...
        if v_rmse < best_val:
            best_val = v_rmse
            torch.save(model.state_dict(), BASE / 'hotend_identifier.pth')
    ...
    model.load_state_dict(torch.load(BASE / 'hotend_identifier.pth'))
...
def obtener_matrices_11_estados(modelo, ventana_datos):
    """
    Recibe la ventana W=5 y devuelve A, B, C, D de 11 estados
    basado en el Jacobiano de la red.
    """
    modelo.eval() # Aseguramos que la red no esté aprendiendo, solo prediciendo

    # Usamos la función matemática que tú ya habías creado en este archivo
    A, B, C, D = extract_state_space(modelo)
...
if __name__ == '__main__':
    ...
    try:
        ...
        np.savez(BASE / 'state_space_hotend.npz', A=A, B=B, C=C, D=D, Ts=Ts)
        sio.savemat(str(BASE / 'state_space_hotend.mat'), {
    ...
    plt.tight_layout()
    plt.savefig(BASE / 'hotend_nn_results.png', dpi=150)
    plt.show()
