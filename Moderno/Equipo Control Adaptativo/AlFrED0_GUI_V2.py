...
class ControlGUI(QWidget):
    ...
    def enviar_lqr_al_arduino(self, K_lista, Nbar):
        ...
        """
        try:
            trama = f"<{K_lista[0]:.4f},{K_lista[1]:.4f},{K_lista[2]:.4f},{K_lista[3]:.4f},{K_lista[4]:.4f},{K_lista[5]:.4f},{Nbar:.4f}>\n"
    ...
    def actualizar(self):
        ...
        # ==============================================================
        # Extraemos las temperaturas y los estados del calentador para la ventana W=5
        min_len = min(len(self.temp_data), len(self.heater_state_data))
        if min_len >= 5:
            ultimos_5_datos = []
            for i in range(min_len - 5, min_len):
                t = self.temp_data[i]
                # Convertimos el estado (0 o 1) a PWM aproximado (0 o 255)
                pwm = self.heater_state_data[i] * 255.0 
                ultimos_5_datos.append([t, pwm])
                
            # Inyectamos de forma segura sin trabar la GUI
            self.hilo_adaptativo.actualizar_ventana(ultimos_5_datos)
