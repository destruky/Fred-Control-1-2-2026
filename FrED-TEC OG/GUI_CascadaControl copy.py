...
class ControlGUI(QWidget):
    def __init__(self):
        ...
        # --- Parámetros de visión (tomados del segundo script) ---
        ...
        self.timer_camara.start(50)  # 20 fps
    ...
    # ------------------------------------------------------------
    # Visión mejorada (integrada desde el segundo script)
    # ------------------------------------------------------------
    def actualizar_imagen_camara(self):
        ...
        if contornos:
            ...
            if cv2.contourArea(main_contorno) < 100:
                # Área demasiado pequeña, no medir
                vis = cv2.resize(frame, (420, 315))
                h, w, ch = vis.shape
                qimg = QImage(vis.data, w, h, ch * w, QImage.Format_RGB888)
                self.label_camara.setPixmap(QPixmap.fromImage(qimg))
                return

            # Usar la cáscara convexa para ignorar entrantes/irregularidades
            hull = cv2.convexHull(main_contorno)
            # Rectángulo mínimo rotado sobre el convex hull
            rect = cv2.minAreaRect(hull)
            cen, tam, ang = rect

            # Asegurar que el ancho sea la dimensión menor
            if tam[0] > tam[1]:
                tam = (tam[1], tam[0])
                ang = ang + 90

            # Matriz de rotación para alinear verticalmente el contorno
            M = cv2.getRotationMatrix2D(cen, ang, 1)

            # Rotar todos los puntos del convex hull
            ...
            # Separar puntos izquierdos y derechos
            p_izq = puntos_rotados[puntos_rotados[:, 0] < centro_x]
            p_der = puntos_rotados[puntos_rotados[:, 0] >= centro_x]

            if len(p_izq) < 5 or len(p_der) < 5:
                # No hay suficientes puntos para medir
                vis = cv2.resize(frame, (420, 315))
                h, w, ch = vis.shape
                qimg = QImage(vis.data, w, h, ch * w, QImage.Format_RGB888)
                self.label_camara.setPixmap(QPixmap.fromImage(qimg))
            ...
            # --- Medición por filas (agrupación en Y) ---
            # Determinar rango vertical útil (superposición de ambos lados)
            y_min = max(p_izq[:, 1].min(), p_der[:, 1].min())
            y_max = min(p_izq[:, 1].max(), p_der[:, 1].max())

            if y_max - y_min < 10:
                # Muy poca altura, no confiable
                vis = cv2.resize(frame, (420, 315))
                h, w, ch = vis.shape
                qimg = QImage(vis.data, w, h, ch * w, QImage.Format_RGB888)
                self.label_camara.setPixmap(QPixmap.fromImage(qimg))
            ...
            # Número de niveles (filas) en los que dividir
            num_niveles = 20
            ...
            for i in range(len(niveles_y) - 1):
                y_inf = niveles_y[i]
                y_sup = niveles_y[i + 1]
                # Puntos izquierdos en esta banda
                mask_izq = (p_izq[:, 1] >= y_inf) & (p_izq[:, 1] < y_sup)
                # Puntos derechos en esta banda
                mask_der = (p_der[:, 1] >= y_inf) & (p_der[:, 1] < y_sup)

                if np.any(mask_izq) and np.any(mask_der):
                    x_izq_mean = np.mean(p_izq[mask_izq, 0])
                    x_der_mean = np.mean(p_der[mask_der, 0])
                    ancho_px = x_der_mean - x_izq_mean
                    if ancho_px > 0:
            ...
            # Filtrado de outliers (eliminar valores extremos)
            anchos = np.array(anchos)
            q1, q3 = np.percentile(anchos, [25, 75])
            iqr = q3 - q1
            filtro = (anchos > q1 - 1.5 * iqr) & (anchos < q3 + 1.5 * iqr)
            if np.any(filtro):
                anchos_filtrados = anchos[filtro]
            else:
                anchos_filtrados = anchos
