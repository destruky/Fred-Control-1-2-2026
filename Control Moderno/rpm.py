...
class ControlGUI(QWidget):
    def __init__(self):
        ...
        # --- Entradas de control u(t) ---
        self.pwm_hotend_data = []
        self.pwm_motor_data  = []
        self.pwm_fan_data    = []
        self.sp_temp_data    = []
        self.sp_rpm_data     = []
        ...
        try:
            while arduino.in_waiting:
                try:
                    ...
                    elif line.startswith("PWM_Hotend:"):
                        self.pwm_hotend_data.append(int(line.split(':',1)[1]))
                    elif line.startswith("PWM_Motor:"):
                        self.pwm_motor_data.append(int(line.split(':',1)[1]))
                    elif line.startswith("PWM_Fan:"):
                        self.pwm_fan_data.append(int(line.split(':',1)[1]))
                    elif line.startswith("SP_Temp:"):
                        self.sp_temp_data.append(float(line.split(':',1)[1]))
                    elif line.startswith("SP_RPM:"):
                        self.sp_rpm_data.append(float(line.split(':',1)[1]))
                    ...
                    # --- Monitor en tiempo real: imprime al cerrar cada bloque de datos ---
                    elif line.startswith("----"):
                        temp  = self.temp_data[-1]          if self.temp_data          else 0.0
                        rpm   = self.motor_rpm_data[-1]     if self.motor_rpm_data     else 0.0
                        pwm_h = self.pwm_hotend_data[-1]    if self.pwm_hotend_data    else 0
                        pwm_m = self.pwm_motor_data[-1]     if self.pwm_motor_data     else 0
                        pwm_f = self.pwm_fan_data[-1]       if self.pwm_fan_data       else 0
                        sp_t  = self.sp_temp_data[-1]       if self.sp_temp_data       else 0.0
                        sp_r  = self.sp_rpm_data[-1]        if self.sp_rpm_data        else 0.0
                        n     = len(self.pwm_motor_data)
                        print(f"[{n:>4}]  "
                              f"Temp={temp:>6.2f}°C  RPM={rpm:>6.2f}  |  "
                              f"PWM → Hotend={pwm_h:>3}  Motor={pwm_m:>3}  Fan={pwm_f:>3}  |  "
                              f"SP → Temp={sp_t:>5.1f}°C  RPM={sp_r:>5.1f}")
    ...
    def export_csv(self):
        ...
        all_data = [
            ...
            self.pwm_hotend_data, self.pwm_motor_data, self.pwm_fan_data,
            self.sp_temp_data, self.sp_rpm_data
        ]
        ...
        try:
            with open(path, 'w', newline='') as f:
                ...
                writer.writerow([
                    'Timestamp',
                    'Temperatura_Hotend', 'RPM_Motor_DC', 'Grosor_Filamento',
                    'PWM_Hotend', 'PWM_Motor_DC', 'PWM_Fan',
                    'Setpoint_Temp', 'Setpoint_RPM',
                ...
                for i in range(max_rows):
                    row = [
                        ...
                        self.pwm_hotend_data[i]        if i < len(self.pwm_hotend_data)        else '',
                        self.pwm_motor_data[i]         if i < len(self.pwm_motor_data)         else '',
                        self.pwm_fan_data[i]           if i < len(self.pwm_fan_data)           else '',
                        self.sp_temp_data[i]           if i < len(self.sp_temp_data)           else '',
                        self.sp_rpm_data[i]            if i < len(self.sp_rpm_data)            else '',
